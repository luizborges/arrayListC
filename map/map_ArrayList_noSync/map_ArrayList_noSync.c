...
map_t Map_ArrayList_noSync_New_Interface() {
	...
	// insere o this
	map->this = (void*)Map_ArrayList_noSync_New();
...
void *Map_ArrayList_noSyncget (const void *_map, const char *key) {
	...
	if(cell == NULL) {
		Error("Map_ArrayList_noSync: no key match for this map. key = \"%s\"", key);
	}
	return cell->content;
}
...
void *Map_ArrayList_noSyncset (void *_map, const char *key, const void *content) {
	...
	if(cell == NULL) { // no exists this key in map - insert this key in map
